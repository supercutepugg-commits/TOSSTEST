...
export default function MyTasks() {
  ...
  const totalTasks = stores.reduce((s, st) => s + st.pendingReview + st.needsAttention + st.openRisks, 0);
    ...
    <div>
      ...
      {stores.length === 0 ? (
        ...
        <div className="card">
          <table>
            <thead>
              <tr><th>가맹점</th><th>검토대기 발주</th><th>미확인 변경알림</th><th>미처리 리스크</th><th></th></tr>
            </thead>
            <tbody>
              {stores.filter(s => s.pendingReview + s.needsAttention + s.openRisks > 0).map(s => (
                <tr key={s.store_id}>
