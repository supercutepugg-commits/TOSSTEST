...
initDb().then(async () => {
  ...
  // Toss Place 과거/누락 매출 자동 동기화: 토스플레이스 매장 ID가 등록된 가맹점만 3분마다 전체 매출(최근 5년) 재동기화
  // 중복 주문은 onConflict ignore로 무시되므로 매번 전체 기간을 다시 가져와도 안전함
  const runAutoSync = async () => {
    try {
      const stores = await knex('stores').whereNotNull('toss_store_id').where('toss_store_id', '!=', '');
      const toDate = new Date().toISOString().split('T')[0];
      const fromDate = new Date(Date.now() - 5 * 365 * 86400000).toISOString().split('T')[0];
